...
contract RebaseToken is ERC20, Ownable, AccessControl {
    ...
    // Errors /////
    ///////////////
    error RebaseToken_InterestRateCanOnlyDecreases(
    ...
    function balanceOf(
        ...
        uint256 interestEarned = (baseBalance * userInterestRate[_user] * timeElapsed) / (365 days);
        return baseBalance + interestEarned;
    }
    ...
    function _calculateUserAccumulatedInterestSinceLastUpdate(
        address _user
    ) internal view returns (uint256 interestEarned) {
        if (lastUpdatedTimeStamp[_user] == 0) {
            return 0;
        }
