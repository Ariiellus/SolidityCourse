...
contract RebaseTokenTest is Test {
    ...
    function testDepositLinear(uint256 amount) public {
        ...
        // NOTE: use `vm.getBlockTimestamp()` when using `vm.warp` in the same tx (esp. `--via-ir`)
        // because `block.timestamp` can get optimized as if it were constant.
        ...
        vm.warp(startTimestamp + 2 hours);
    ...
    function testInterestRateCanOnlyDecrease(uint256 newInterestRate) public {
        ...
        vm.prank(owner);
        vm.expectPartialRevert(bytes4(RebaseToken.RebaseToken_InterestRateCanOnlyDecreases.selector));
        rebaseToken.setInterestRate(newInterestRate);
